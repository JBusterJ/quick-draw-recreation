...
objToDraw = getObjectToDraw();
document.getElementById("drawObj").innerHTML = objToDraw;
timer();
...
function gotResult(e, results){
    ...
    if(previous != results[0].label && results[0].label.replace("_", " ") != objToDraw){
        utterThis = new SpeechSynthesisUtterance(results[0].label.replaceAll("_", " "));
        synth.speak(utterThis);
    }
    if(results[0].label.replace("_", " ") == objToDraw){
        utterThis = new SpeechSynthesisUtterance("Oh, I got it! It's a " + objToDraw);
        synth.speak(utterThis);
        clearCanvas();
        objToDraw = getObjectToDraw();
        document.getElementById("drawObj").innerHTML = objToDraw;
        score += 10;
        document.getElementById("drawingScore").innerHTML = score;
        timeLeft = 60;
    }
...
function getObjectToDraw(){
    return quick_draw_data_set[Math.floor(Math.random(1) * 345)];
}
